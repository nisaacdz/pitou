...
pub type DirEntryResult = io::Result<fs::DirEntry>;
...
impl File {
    ...
    /// Returns the size of the current file including the sizes of any files or folders within it (as a Directory) in the file tree
    pub fn size_all(&self) -> io::Result<u64> {
        let mut res = self.size();
        if self.is_dir() {
            res += self.entries()?.map(|d| d.weight()).sum::<u64>();
        }
        Ok(res)
    }
}
...
impl<F> FilesIn<F> {
    pub fn new(iter: iter::Map<fs::ReadDir, F>) -> Self {
        Self(iter)
    }
}
